  ...
  MainThread::GetInstance().PostAsync([this, stream_id] {
    for (auto iter = participant_publishs_map_.begin(); iter != participant_publishs_map_.end();
      ...
      auto publish_streams_iter =
          std::find_if(publish_streams.begin(), publish_streams.end(),
                       [stream_id](auto s) { return s->GetStreamId() == stream_id; });

      if (publish_streams_iter != publish_streams.end()) {
        auto notification = Notification::MakeStreamAddedNotification(
            id_, iter->first, (*publish_streams_iter)->GetStreamId(),
            (*publish_streams_iter)->GetSdp());
        SignalingServer::GetInstance().Notify(notification);
      }
    }
    ...
    for (auto publish_subscribes_map_iter = publish_subscribes_map_.begin();
      ...
      for (auto subscribe_stream_set_iter = subscribe_stream_set.begin();
           subscribe_stream_set_iter != subscribe_stream_set.end(); ++subscribe_stream_set_iter) {
    ...
    for (auto iter = participant_subscribes_map_.begin(); iter != participant_subscribes_map_.end();
      ...
      if (subscribe_stream_set_iter != subscribe_stream_set.end()) {
        ...
        for (auto publish_subscribes_map_iter = publish_subscribes_map_.begin();
          ...
          auto publish_stream = publish_subscribes_map_iter->first;
          for (auto subscribe_stream_set_iter = subscribe_stream_set.begin();
               subscribe_stream_set_iter != subscribe_stream_set.end();) {
            if ((*subscribe_stream_set_iter)->GetStreamId() == stream_id) {
