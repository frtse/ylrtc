...
std::optional<double> ComputeSlopeCap(
  ...
  DCHECK(1 <= settings.beginning_packets &&
             settings.beginning_packets < packets.size());
  DCHECK(1 <= settings.end_packets &&
             settings.end_packets < packets.size());
  DCHECK(settings.beginning_packets + settings.end_packets <=
             packets.size());
...
void TrendlineEstimator::UpdateThreshold(double modified_trend,
  ...
  threshold_ = std::clamp(threshold_, (double)6.f, (double)600.f);
  last_update_ms_ = now_ms;
}