...
void PacketArrivalTimeMap::AddPacket(int64_t sequence_number, int64_t arrival_time_ms) {
  if (!has_seen_packet_) {
    // First packet.
  ...
  if (static_cast<size_t>(pos) >= kMaxNumberOfPackets) {
    ...
    if (packets_to_remove >= arrival_times.size()) {
      ...
      while (packets_to_remove < arrival_times.size() && arrival_times[packets_to_remove] == 0) {
        ++packets_to_remove;
      }

      arrival_times.erase(arrival_times.begin(), arrival_times.begin() + packets_to_remove);
  ...
  DCHECK(arrival_times.size() == pos);
  arrival_times.push_back(arrival_time_ms);
  DCHECK(arrival_times.size() <= kMaxNumberOfPackets);
}

void PacketArrivalTimeMap::RemoveOldPackets(int64_t sequence_number, int64_t arrival_time_limit) {
  while (!arrival_times.empty() && begin_sequence_number_ < sequence_number && arrival_times.front() <= arrival_time_limit) {
...
bool PacketArrivalTimeMap::has_received(int64_t sequence_number) const {
  int64_t pos = sequence_number - begin_sequence_number_;
  if (pos >= 0 && pos < static_cast<int64_t>(arrival_times.size()) && arrival_times[pos] != 0) {
    return true;
  }
  return false;
}
...
void PacketArrivalTimeMap::EraseTo(int64_t sequence_number) {
  if (sequence_number > begin_sequence_number_) {
    size_t count = std::min(static_cast<size_t>(sequence_number - begin_sequence_number_), arrival_times.size());
...
int64_t PacketArrivalTimeMap::clamp(int64_t sequence_number) const {
  return std::clamp(sequence_number, begin_sequence_number(), end_sequence_number());
}